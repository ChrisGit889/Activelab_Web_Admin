...
import { MdQrCodeScanner } from "react-icons/md"; // Icon scanner pengganti FiScan
...
export default function CheckOutPage() {
  ...
  const handleStartScan = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/admin/scan/sessions`, {
        ...
        // PERUBAHAN: scan_type diset menjadi checkout
        body: JSON.stringify({ scan_type: "checkout" }),
      });
  ...
  const handleManualSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/admin/scan/sessions/${sessionId}/process`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            manual_code: manualCode.trim().toUpperCase(),
          }),
        }
      );
      const data = await res.json();
  ...
  return (
    <div className="container-fluid py-4">
      <h4 className="fw-bold mb-1">Scan Check-Out</h4>
      <p className="text-muted mb-4">
        Proses check-out member menggunakan QR code
      </p>
      ...
      {!sessionId && !scanResult && (
        <div className="card border-0 shadow-sm" style={{ maxWidth: 500 }}>
          <div className="card-body p-4 text-center">
            <div
              // PERUBAHAN: Warna diubah ke bg-success
              className="rounded-circle bg-success bg-opacity-10 d-flex align-items-center justify-content-center mx-auto mb-3"
              style={{ width: 80, height: 80 }}
            >
              <MdQrCodeScanner size={36} color="#198754" />
            </div>
            <h5 className="fw-bold mb-2">Mulai Scan Check-out</h5>
            <p className="text-muted small mb-4">
              Klik tombol di bawah, lalu scan QR yang muncul dengan HP admin
              untuk membuka scanner.
            </p>
            <button
              // PERUBAHAN: Tombol diubah menjadi btn-success
            ...
            <div className="card border-0 shadow-sm">
              <div className="card-body p-4 text-center">
                ...
                <p className="text-muted small mb-3">
                  HP akan membuka halaman scanner
                </p>
                {scannerUrl && (
                ...
                <div className="mt-2 d-flex align-items-center gap-2 justify-content-center">
                  {/* PERUBAHAN: Spinner menjadi variant success */}
                  <Spinner animation="border" size="sm" variant="success" />
                  <span className="text-muted small">
                    Menunggu scan dari HP admin...
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="col-md-6">
            <div className="card border-0 shadow-sm">
              <div className="card-body p-4">
                ...
                <p className="text-muted small mb-3">
                  Jika QR tidak bisa di-scan, minta user untuk memberikan kode 6
                  huruf mereka.
                </p>
                {manualError && (
          ...
          <div className="card-body p-4 text-center">
            {scanResult.success ? (
              <>
                ...
                <div className="card bg-light border-0 mt-3 text-start">
                  <div className="card-body p-3">
                    <p className="mb-1">
                      <strong>User:</strong> {scanResult.booking?.user_name}
                    </p>
                    <p className="mb-1">
                      <strong>Email:</strong> {scanResult.booking?.user_email}
                    </p>
                    <p className="mb-1">
                      <strong>Layanan:</strong>{" "}
                      {scanResult.booking?.service_type_name} -{" "}
                      {scanResult.booking?.service_name_name}
                    </p>
                    <p className="mb-1">
                      <strong>Ruangan:</strong> {scanResult.booking?.room_name}
                    </p>
                    <p className="mb-0">
                      <strong>Jadwal:</strong> {scanResult.booking?.sched_date}{" "}
                      {scanResult.booking?.sched_start} -{" "}
                      {scanResult.booking?.sched_end}
                    </p>
                  </div>
                </div>
              </>
              ...
              <>
                ...
                <h5 className="fw-bold text-danger mb-2">Check-out Gagal</h5>
